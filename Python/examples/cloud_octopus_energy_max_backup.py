...
def evaluate_planned_dispatches(current_ts, octopus_planned_dispatches):
    # There may be no planned dispatches.
    if not octopus_planned_dispatches:
        return None
...
def send_message(owner_api, energy_site_id, private_key, public_key_bytes, gateway_din, teg_message, debug=True):
...
def decide_max_backup_action(current_ts, max_backup_until, planned_dispatch_until):
    """
    Decide what Max Backup action to take based on current and planned dispatch times.

    Args:
        current_ts (int): Current timestamp in seconds.
        max_backup_until (int | None): End timestamp of the current Max Backup, or None.
        planned_dispatch_until (int | None): End timestamp of the planned dispatch, or None.

    Returns:
        list: Messages to send (may be empty).
    """

    # Determine whether to start, reset, stop Max Backup on the Tesla® Gateway (or do nothing).
    messages_to_send = []

    # We are in a planned dispatch time and Max Backup is not currently set.
    if planned_dispatch_until and not max_backup_until:
        # Notify the user.
    ...
    elif not planned_dispatch_until and max_backup_until:
        # Notify the user.
...
def main():
    ...
    # Get the Octopus Energy® Flex Device ID.
    device_id = get_or_update_octopus_energy_device_id(configuration, octopus_energy)

    # Get the Octopus Energy® Flex Planned Dispatches.
    octopus_planned_dispatches = query_octopus_energy_graphql(octopus_energy, device_id)

    # Evaluates the planned dispatches (if any) and returns when the current one ends.
