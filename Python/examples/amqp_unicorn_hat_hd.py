            ...
            while True:
                ...
                if timestamp > time.time()-5:
                    ...
                    if state_of_energy is not None:
                        # The gateway scales the state of energy to reserve a 5% buffer.
                        scaled_state_of_energy = Gateway.scale_soe(state_of_energy)

                        # Round the number to 2 decimal places (so 1.999 becomes 2.00 etc).
                        # Ensure the result is non-negative; clip to 0.0 if it's less than 0.
                        scaled_state_of_energy = max(0.0, round(scaled_state_of_energy, 2))
